...
Load the data set as a `data.frame` and name it `rosca`.
...
knitr::kable(table(rosca$treatment), col.names = c("Treatment Condition", "N"))
```
...
Create a matrix named `rosca_dropout` where the first column contains the number of participants left in each group,
and the second column contains the drop-out rate across different treatment conditions.
Generate a nice-looking table using `knitr::kable()`
...
# left in the follow-up 
n_follow_up  <- table(rosca2$treatment)  
rate_dropout <- (table(rosca$treatment)-table(rosca2$treatment))/table(rosca$treatment)

# create a table 
rosca_dropout <- cbind(n_follow_up, rate_dropout)
knitr::kable(rosca_dropout, 
             col.names = c("N in the follow-up", "Drop-out Rate"),
             digits = 2)
```
...
Does receiving a safe box increase the amount invested in health products? We focus on the outcome measured 12 months from baseline during the second follow-up. Compare the mean of amount (in Kenyan shilling) invested in health products `fol2_amtinvest` between each of the treatment arms and the control group. Save the computed differences as an object named `ate`. Briefly interpret the result.

**Hint**: Use `tapply()` to compute the group mean.

## Answer 3

```{r}
## compute group mean 
group_mean <- tapply(rosca2$fol2_amtinvest, rosca2$treatment, mean)
ate <- group_mean[c("safebox", "lockbox")] - group_mean["control"]
```
...
Compute the group mean for each variable using `tapply()`. 
Save the computed means as a matrix named `covar_balance` where 
the first column stores the group mean of `bf_female`, 
the second column stores the group mean of `bg_b1_age`,
and the last column stored the group mean of `bf_married`.
Report this matrix as a nice-looking table using `knitr::kable()`.
...
# report a table
knitr::kable(covar_balance, 
             col.names = c("Female", "Age", "Married"),
             digits = 2)
```
...
# reprt 
knitr::kable(
  cbind(ate_married, ate_unmarried),
  col.names = c("Married", "Unmarried"),
  digits = 2
)
```
